...
- In a dataset we observe _at most_ half of the vector $Y^a$ because we can only observe each person as $Y^{a=1}$ or $Y^{a=0}$
- We write the causal effect we're interested in as $Y^{a=1}-Y^{a=0}$ or $\frac{Pr[Y^{a=1}=1]}{Pr[Y^{a=0}=1]}$
...
- This DAG implies that if $Z \not\!\perp\!\!\!\perp Y$ then it must also be that $A \not\!\perp\!\!\!\perp Y^a$
- IV estimation requires monontonicity and homogeneity cannot be drawn on a DAG because they are parametric concepts
...
- But if we do, now $Z \not\!\perp\!\!\!\perp U$ and therefore $Z \not\!\perp\!\!\!\perp Y$, AKA, the exchangeability assumption is violated
...
- We often draw this on DAGs using a dashed line to remind us that these variables are associated now
